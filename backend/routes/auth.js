...
const storage = multer.diskStorage({
  destination: function (req, file, cb) {
    const uploadPath = path.join(__dirname, '../public/profilepictures');
  ...
  filename: function (req, file, cb) {
    const userId = req.userParams?.userId || 'default';
...
// Feltételezve, hogy van user id a kérésben (auth tokenből vagy paraméterből)
router.post('/upload-avatar', [authJwt.verifyToken, upload.single('avatar')], async (req, res) => {
  try {
    const userId = req.userParams.userId;
    const file = req.file;

    if (!file) {
      return res.status(400).json({ error: 'Nem küldtél fájlt.' });
    }

    const filename = file.filename;

    // Mentés az adatbázisba
    await Db.updateAvatar(userId, filename);

    res.json({ success: true, filename, message: 'Avatar feltöltve/frissítve.' });
  } catch (error) {
    console.error('Avatar feltöltési hiba:', error);
    res.status(500).json({ error: 'Hiba történt az avatar feltöltése során.' });
  }
});
