...
async function updateAvatar(userId, filename) {
  return new Promise((resolve, reject) => {
    const sql = `UPDATE felhasznalok SET avatar = ? WHERE felhasznalo_id = ?`;
    pool.query(sql, [filename, userId], (error, results) => {
      if (error) {
...
async function loginUser(email, password) {
  return new Promise((resolve, reject) => {
    pool.query(
      'SELECT felhasznalo_id,felhasznalo_nev,felhasznalostatusz,jog,email,regisztracio_datum,utoljara_belepve,avatar FROM felhasznalokinfo WHERE email = ? AND jelszo=SHA2(?,256)',
      [email, password],
      (error, elements) => {
        if (error) {
          reject(error);
        ...
        pool.query(
          'UPDATE felhasznalok SET utoljara_belepve = ? WHERE felhasznalo_id = ?',
          [mysqlDate, user.felhasznalo_id],
          (updateError) => {
            if (updateError) {
              reject(updateError);
              return;
            }
            user.utoljara_belepve = mysqlDate;
            resolve([user]);
          }
        );
      }
